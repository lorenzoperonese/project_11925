...
void replaceSymbols(char *line, symbolTable *st) {
    ...
    if(line[0] == '@' && line[1] != '\0' && !(line[1] <= '9' && line[1] >= '0')) {
        line++;
        printf("simbolo %s\n",line);
        if(findBySymbol(line,st)==MISSING){
            printf("simbolo non trovato %s\n",line);
            addSymbol(line,varaddr,st);
            varaddr++;
        }
        sprintf(line,"%u",findBySymbol(line,st));
